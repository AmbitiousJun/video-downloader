...
public class FreeApiSeleniumDecoder extends SeleniumDecoder{
    ...
    public String fetchDownloadLink(String fetchUrl) {
        LogUtils.info(logger, "正在获取视频的真实下载链接...{}" + formatLogSuffix(fetchUrl));
        // 1 判断客户端是否初始化
        ...
        LogUtils.info(logger, String.format("等待解析完成... %s 秒,%s", Config.DECODER.FREE_API.WAIT_SECONDS, formatLogSuffix(fetchUrl)));
        try {
            Thread.sleep(Config.DECODER.FREE_API.WAIT_SECONDS * 1000);
        } catch (InterruptedException e) {
            throw new RuntimeException("解析被中断," + formatLogSuffix(fetchUrl));
        }
        LogUtils.info(logger, String.format("尝试获取视频下载链接...,%s", formatLogSuffix(fetchUrl)));
        // 3 分析抓包文件，提取下载链接
        ...
        for (HarEntry entry : entries) {
            ...
            for (String validPrefix : Config.DECODER.FREE_API.VALID_URL_PREFIXES) {
                if (possibleUrl.contains(validPrefix)) {
                    LogUtils.success(logger, String.format("成功获取到下载链接：%s,%s", possibleUrl, formatLogSuffix(fetchUrl)));
                    return possibleUrl;
                }
            }
        }
