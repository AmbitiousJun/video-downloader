...
public abstract class M3U8Actuator implements DownloadActuator {
    ...
    public void download(DownloadMeta meta) throws Exception {
        // 读取 ts 文件
        LOGGER.info("正在读取 ts 文件列表, 目标视频：{}", meta.getFileName());
        Deque<TsMeta> tsMetas = M3U8Utils.readTsUrls(meta.getLink(), meta.getHeaderMap());
        int size = tsMetas.size();
        LOGGER.info("读取完成，共发现 {} 个 ts 文件，目标视频：{}", size, meta.getFileName());
        if (size == 0) {
            LOGGER.info("这是一个空的 m3u8 文件，下载取消，目标视频：{}", meta.getFileName());
            return;
        }
        // 初始化临时文件夹
        ...
        LOGGER.info("视频下载完成，目标视频：{}", meta.getFileName());
    }
